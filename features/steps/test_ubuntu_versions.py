...
@step("I get the version of the default node command")
def get_node_version(context) -> None:
    run_docker_command(context, "node --version")
...
@step("it is version '(.*)'")
def check_node_version(context, container_name: str) -> None:
    m = VERSION_MATCHER.match(context.docker_output)

    if not m:
        raise Exception("Unable to parse version from: `%s`" % context.docker_output)

    context.node_version = m.group(2)

    assertEqual(container_name, context.node_version)
