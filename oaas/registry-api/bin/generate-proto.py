...
def find_target_file(grpc_file: str, suffix: str) -> str:
    ...
    return os.path.join("oaas_registry_api", "rpc", python_file)
...
def grpc_compile(
    ...
    subprocess.check_call(
        [
            ...
            "-I",
            FOLDER,
            "--python_out=oaas_registry_api/rpc/",
            "--grpc_python_out=oaas_registry_api/rpc",
            "--mypy_out=oaas_registry_api/rpc",
            grpc_file,
        ]
    )
    ...
    with open(generated_grpc_file, "rt", encoding="utf-8") as f:
        ...
        new_content = IMPORT_RE.sub(r"\1 oaas_registry_api.rpc.\2", content)
