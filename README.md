...
them in order, and has a process defined as `<start>` → `Checkout Code`
→ `Run
Build` → `<end>`.
...
Note that you can pass `initial_data` into the process, and you can also
get the `context.data` from the last execution token.
...
![BPMN Editor](./doc/yaoqiang-screenshot.png)

The [build of adhesive](_adhesive.py) is modeled as a [BPMN
process](adhesive-self.bpmn) itself, so we load it from the file
...
process. Only the `adhesive.build()` creates a linear process out of the
...
3.  `data` - Data that the current execution token contains. This data
    ...
    have a modifiable copy of the data that you’re allowed to change,
    and is propagated into the following execution tokens.

4.  `loop` - if the current task is in a loop, the entry contains its
    `index`, the `key` and `value` of the items that are iterating, and
    the `expression` that was evaluated. Note that loop execution
...
6.  `workspace` - a way to interact with a system, and execute commands,
    create files, etc.
...
access to the `task`, `task_name`, `data`, `loop`, `lane` and `context`,
as well as the variables defined in the `context.data`.
...
The `navigation_direction` can be validated in the condition with any of
the following:
...
API is an API that allows you to run commands, and create files, taking
...
form elements that are populated in the `context.data`, and available in
subsequent tasks.
...
Implicitly a single button with an `OK` label is added to the User Task,
that when pressed fills the `context.data` in the outgoing execution
token.

With `add_default_button` we create custom buttons such as `Back` and
`Forward`, or whatever we need in our process. Unlike the default `OK`
button, when these are called, they also set in the `context.data` the
`value` that’s assigned to them. This value we use then further in a
`Gateway`, or simple as a condition on the outgoing edges.
...
assign a different variable in the `context.data`, or from the `value`
if they have overlapping names.
...
In order to make them available, we just use the `secret` function that
creates the file in the current workspace and deletes it when exiting.
For example here’s how we’re doing the actual publish, creating the
secret inside a docker container:
...
this is `true`, but if log parsing is needed, it can make sense to have
it false.
...
This value can be set to `process`, in case the tasks are CPU intensive.
This has the drawback of recreating the connections on workspaces' each
task execution.
...
the [adhesive-lib](https://github.com/germaniumhq/adhesive-lib) shared
plugin, and configure your local config to use it:
